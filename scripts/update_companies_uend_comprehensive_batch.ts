...
async function updateUENDComprehensiveCompanies() {
  try {
    console.log('Processing U-END comprehensive companies batch...\n');

    // Read and parse CSV file
    const csvContent = readFileSync('attached_assets/u-end-eng-1-swe_1753987079608.csv', 'utf-8');
    ...
    console.log('\n=== U-END Comprehensive Companies Update Summary ===');
    ...
    console.log('\n=== Processing Complete ===');
    console.log(`Database now contains ${updatedCount} additional companies with comprehensive bilingual descriptions`);

  } catch (error) {
    console.error('Error updating U-END comprehensive companies batch:', error);
  } finally {
    await pool.end();
  }
}

updateUENDComprehensiveCompanies();