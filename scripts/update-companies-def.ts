...
async function updateCompaniesDEF() {
  try {
    console.log("🔄 Starting D-E-F company data update process...");
    
    // Read the cleaned DEF updates JSON file
    const jsonPath = path.join(process.cwd(), "attached_assets/updatesDEF_cleaned.json");
    ...
    for (const companyUpdate of updatedCompanies) {
      try {
        // Skip companies with empty descriptions
        if (!companyUpdate.description || companyUpdate.description.trim() === "") {
          console.log(`⏭️  Skipping ${companyUpdate.name} - no description provided`);
          skippedCount++;
          continue;
        }
    ...
    console.log(`⏭️  Companies skipped (no description): ${skippedCount}`);
...
updateCompaniesDEF()
  .then(() => {
    console.log("🎉 D-E-F company update completed successfully!");
    process.exit(0);
  })
  .catch((error) => {
    console.error("💥 D-E-F company update failed:", error);
    process.exit(1);
  });