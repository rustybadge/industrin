...
async function updateNOCompanies() {
  try {
    console.log('Processing N-O companies batch...\n');

    // Read and parse CSV file
    const csvContent = readFileSync('attached_assets/NO-Companies_MINIFORREPLIT_1753976895412.csv', 'utf-8');
      ...
      try {
        ...
        if (existingCompany) {
          ...
          if (record.description_sv && record.description_sv.trim() !== '') {
            updateData.descriptionSv = record.description_sv;
          }
          ...
          if (Object.keys(updateData).length > 0) {
            ...
            if (updateData.descriptionSv) descTypes.push('Swedish');
    ...
    if (updatedCount > 0) {
      console.log('\n=== Verification - Updated Companies ===');
    ...
    console.error('Error updating N-O companies:', error);
  } finally {
    await pool.end();
  }
}
