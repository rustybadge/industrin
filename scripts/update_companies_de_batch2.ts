...
async function updateCompanies() {
  try {
    // Read the CSV file
    const csvContent = fs.readFileSync('attached_assets/DE_companies_eng_swe_REPLIT_1753877829847.csv', 'utf-8');
    ...
    console.log(`Starting to update ${records.length} D-E companies (batch 2) from CSV file...`);
    ...
    console.log('\n=== D-E Companies (Batch 2) Update Summary ===');
    ...
    console.log('\n=== Sample D-E Company Updates (Batch 2) ===');
    ...
    console.error('Error updating D-E companies (batch 2):', error);
  } finally {
    await pool.end();
  }
}
